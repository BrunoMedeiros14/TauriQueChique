...
fn cliente_from_model_db(cliente_db: ClienteDb) -> Cliente {
  Cliente {
    ...
    data_nascimento: cliente_db.data_nascimento.map(|data|
      NaiveDate::parse_from_str(&data, "%Y-%m-%d").unwrap()
    ),
...
fn model_db_from_cliente(cliente: Cliente) -> ClienteDb {
  ClienteDb {
    ...
    data_nascimento: cliente.data_nascimento.map(|date| date.format("%Y-%m-%d").to_string()),
...
pub fn buscar_cliente_por_id(cliente_id: i32, conn: &Connection) -> Cliente {
  ...
  cliente_db.map(|cliente_db| cliente_from_model_db(cliente_db)).unwrap()
}
...
pub fn buscar_todos_clientes(conn: &Connection) -> Result<Vec<Cliente>> {
  ...
  let clientes = rows.map(|row| cliente_from_model_db(row.unwrap())).collect::<Vec<_>>();
  Ok(clientes)
}
